...
class Converter
{
     ...
     * @return string
     */
    protected function replaceWithRule(rules, output) -> string
    {
    	var rule, from, to;

    	for rule in rules {	
    		let from = "~" . json_decode("\"" . rule["from"] . "\"") . "~u";
    		let to = json_decode("\"" . rule["to"] . "\"");

    		let output = preg_replace(from, to, output);
    	}
